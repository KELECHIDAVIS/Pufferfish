public class Perft {
    ...
    public static int perft(Board board, int depth, int maxDepth, Dictionary<ulong, int> boardMemo) {
        ...
        //if zobrist seen before in memo just return that value
        if(boardMemo.ContainsKey(board.state.zobristKey))
            return boardMemo[board.state.zobristKey];
        ...
        foreach (Move move in moves) {
            ...
            int childResponses=  perft(child, depth+1, maxDepth, boardMemo);
        ...
        // save the total moves for this board in the zobrist key
        boardMemo.Add(board.state.zobristKey, totalMoves); 
        return totalMoves; 
    }
